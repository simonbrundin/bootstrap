...
if ! gh auth status &> /dev/null; then
    echo "Du är inte inloggad. Kör: gh auth login"
    exit 1
fi

# === 2. Hämta befintliga nycklar (endast autentiseringsnycklar) ===
echo "Hämtar befintliga SSH-nycklar från GitHub..."
AUTH_KEYS_JSON=$(gh api "/user/keys" --silent) || {
    echo "Kunde inte hämta nycklar. Kontrollera internet/inloggning."
    exit 1
}

# Kontrollera om nyckeln redan finns
if echo "$AUTH_KEYS_JSON" | jq -e ".[] | select(.title == \"$KEY_NAME\")" >/dev/null; then
...
if [[ -z "$PUB_KEY" ]]; then
    echo "Kunde inte läsa publik nyckel!"
    exit 1
fi
